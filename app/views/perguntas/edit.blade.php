...
@section('content')
	<h4>
		<span class="glyphicon glyphicon-edit"></span> Alterar pergunta
		<a href="{{ URL::to('pergunta') }}" class="btn btn-info navbar-right"><span class="glyphicon glyphicon-chevron-left"></span> Voltar</a>
	</h4>
	<hr>
	{{ Form::open(array('url' => 'pergunta/' . $pergunta->id, 'method' => 'put', 'class' => 'form-horizontal row', 'role' => 'form', 'files'=>true)) }}

				<div class="col-xs-4 {{ $errors->first('categoria_id') ? 'has-error' : '' }}">
			{{ Form::label('categoria_id', '* Categoria', array('class' => 'control-label')) }}
        	{{ Form::select('categoria_id', Categoria::options(), Input::old('categoria_id', $pergunta->categoria_id), array('class' => 'form-control', 'required')) }}
		...
		<div class="col-xs-4 {{ $errors->first('nivel_id') ? 'has-error' : '' }}">
        	...
        	{{ Form::select('nivel_id', Nivel::options(), Input::old('nivel_id',$pergunta->nivel_id), array('class' => 'form-control', 'required')) }}
		...
		<div class="col-xs-6 pull-left {{ $errors->first('nome') ? 'has-error' : '' }}">
        	...
        	{{ Form::textarea('nome', Input::old('nome', $pergunta->nome), array('class' => 'form-control', 'required')) }}
		...
		<div class="col-xs-4 {{ $errors->first('resposta1') ? 'has-error' : '' }}">
        	...
        	{{ Form::text('resposta1', Input::old('resposta1' , $pergunta->resposta1), array('class' => 'form-control', 'required')) }}
		...
		<div class="col-xs-4 {{ $errors->first('resposta2') ? 'has-error' : '' }}">
        	...
        	{{ Form::text('resposta2',  Input::old('resposta2' , $pergunta->resposta2), array('class' => 'form-control', 'required')) }}
		...
		<div class="col-xs-4 {{ $errors->first('resposta3') ? 'has-error' : '' }}">
        	...
        	{{ Form::text('resposta3', Input::old('resposta3' , $pergunta->resposta3), array('class' => 'form-control', 'required')) }}
        	{{ $errors->first('nivel_id', '<span class="text-danger">:message</span>') }}
		</div>
		<div class="col-xs-4 {{ $errors->first('resposta4') ? 'has-error' : '' }}">
			{{ Form::label('resposta4', '* Alternativa 4', array('class' => 'control-label')) }}
        	{{ Form::text('resposta4', Input::old('resposta4', $pergunta->resposta4), array('class' => 'form-control', 'required')) }}
        	{{ $errors->first('nivel_id', '<span class="text-danger">:message</span>') }}
		</div>
		<div class="col-xs-4 {{ $errors->first('respostaCerta') ? 'has-error' : '' }}">
			{{ Form::label('respostaCerta', '* Resposta Certa', array('class' => 'control-label')) }}
        	{{ Form::select('respostaCerta', Pergunta::opcoes(), Input::old('respostaCerta', $pergunta->respostaCerta), array('class' => 'form-control select2', 'required')) }}
		...
		<div class="col-xs-4 {{ $errors->first('dica') ? 'has-error' : '' }}">
        	...
        	{{ Form::text('dica', Input::old('dica', $pergunta->dica), array('class' => 'form-control', 'required')) }}
		...
		{{ Form::hidden('poster_atual', $pergunta->poster) }}
