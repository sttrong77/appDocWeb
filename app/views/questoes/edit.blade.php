...
@section('content')
	<h4>
		<span class="glyphicon glyphicon-edit"></span> Alterar questao
		<a href="{{ URL::to('questoes') }}" class="btn btn-info navbar-right"><span class="glyphicon glyphicon-chevron-left"></span> Voltar</a>
	</h4>
	<hr>
	{{ Form::open(array('url' => 'questoes/' . $questao->id, 'method' => 'put', 'class' => 'form-horizontal row', 'role' => 'form')) }}
		...
		<div class="col-xs-6 pull-left {{ $errors->first('nome_questao') ? 'has-error' : '' }}">
        	...
        	{{ Form::textarea('nome_questao', Input::old('nome_questao', $questao->nome_questao), array('class' => 'form-control', 'required')) }}
		...
		<div class="col-xs-4 {{ $errors->first('resposta1') ? 'has-error' : '' }}">
        	...
        	{{ Form::text('resposta1', Input::old('resposta1' , $questao->resposta1), array('class' => 'form-control', 'required')) }}
		...
		<div class="col-xs-4 {{ $errors->first('resposta2') ? 'has-error' : '' }}">
        	...
        	{{ Form::text('resposta2',  Input::old('resposta2' , $questao->resposta2), array('class' => 'form-control', 'required')) }}
		...
		<div class="col-xs-4 {{ $errors->first('resposta3') ? 'has-error' : '' }}">
        	...
        	{{ Form::text('resposta3', Input::old('resposta3' , $questao->resposta3), array('class' => 'form-control', 'required')) }}
		...
		<div class="col-xs-4 {{ $errors->first('resposta4') ? 'has-error' : '' }}">
        	...
        	{{ Form::text('resposta4', Input::old('resposta4', $questao->resposta4), array('class' => 'form-control', 'required')) }}
        	{{ $errors->first('nivel_id', '<span class="text-danger">:message</span>') }}
		</div>
		<div class="col-xs-4 {{ $errors->first('resposta') ? 'has-error' : '' }}">
			{{ Form::label('resposta', '* Resposta Certa', array('class' => 'control-label')) }}
        	{{ Form::select('resposta', Questoes::opcoes(), Input::old('resposta', $questao->resposta), array('class' => 'form-control select2', 'required')) }}
        	{{ $errors->first('resposta', '<span class="text-danger">:message</span>') }}
		</div>
